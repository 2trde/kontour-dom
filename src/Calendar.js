...
const Day = ({value, currentDay, onSelectDay}) => {
  ...
  return <div style={style} key={'' + value.date() + '_' + value.month()} onClick={() => onSelectDay(value)}>
    {value.date()}
  </div>
}
...
function chunkArray(arr, chunkSize) {
  ...
  arr.forEach((el) => {
    ...
    if (list[list.length - 1].length >= chunkSize) { list.push([]) }
    list[list.length - 1].push(el)
  })
  return list
}

const DaysRow = ({children, style}) => {
...
class Calendar extends Component {
  ...
  currentDate() {
    if (this.props.value) {
      if (this.props.value.isValid()) { return this.props.value } else { return moment() }
    } else {
      return moment()
    }
  }
  ...
  render() {
    ...
    return <div style={style}>
      ...
      <DaysRow style={{background: 'lightblue'}}>
        {weekDays.map((wday) => <div key={wday} style={weekDayStyle}>{wday}</div>)}
      </DaysRow>
      <DaysRow>
        {days.map((d) => <Day key={'' + d.date() + '_' + d.month()} value={d} currentDay={this.currentDate()} onSelectDay={() => this.onSelectDay(d)} />)}
      </DaysRow>
    </div>
  }
}
